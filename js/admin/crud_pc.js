document.addEventListener("DOMContentLoaded", function () {
    ...
    function debugTabla() {
        ...
        console.log(`Tabla de PCs encontrada. Número de filas: ${filas.length}`);
    }
    ...
    function verificarAsignacion(id_activo) {
        fetch(`../controllers/procesar_pc.php?verificar_asignacion=1&id_activo=${id_activo}`)
            ...
            .then(data => {
                if (data.asignado) {
                    alert("Este PC está asignado actualmente. Algunas opciones de edición pueden estar limitadas.");
                }
            })
    ...
    document.querySelectorAll('.btn-qr-generate, .btn-qr-regenerate').forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            if (!idActivo) {
                alert('No se pudo identificar el ID del PC');
                return;
            }
