document.addEventListener("DOMContentLoaded", function () {
    ...
    function debugTabla() {
        ...
        if (!tabla) {
            console.error("No se encontró la tabla de Servidores en el DOM");
            return;
        }

        const filas = tabla.querySelectorAll("tbody tr");
        console.log(`Tabla de Servidores encontrada. Número de filas: ${filas.length}`);
    }
    ...
    function configurarToggleObservaciones() {
        ...
        if (btnToggle && contenedor) {
            ...
            console.log("Toggle de observaciones configurado correctamente para Servidor");
        }
    }
    ...
    window.agregarComponente = function (tipo) {
        ...
        btnEliminar.onclick = () => {
            componentesSeleccionados[tipo].delete(div.dataset.id);
            div.remove();
    ...
    if (btnNuevo) {
        btnNuevo.addEventListener("click", function () {
            ...
            if (modalTitle) modalTitle.textContent = "Registrar Servidor";
    ...
    const filas = document.querySelectorAll("#tablaServidores tbody tr");
    if (buscador) {
    ...
    // Función para verificar asignación
    function verificarAsignacion(id_activo) {
        fetch(`../controllers/procesar_servidor.php?verificar_asignacion=1&id_activo=${id_activo}`)
            .then(response => response.json())
            .then(data => {
                if (data.asignado) {
                    alert("Este Servidor está asignado actualmente. Algunas opciones de edición pueden estar limitadas.");
                }
            })
    ...
    document.querySelectorAll('.btn-qr-generate, .btn-qr-regenerate').forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            if (!idActivo) {
                alert('No se pudo identificar el ID del Servidor');
                return;
            }
    ...
    window.addEventListener('load', function() {
        debugTabla();
        console.log("Sistema de gestión de Servidores cargado correctamente");
    });
});