...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($accion === "crear") {
        ...
        try {
            ...
            $cpus = isset($_POST['cpus']) ? explode(',', $_POST['cpus']) : [];
            $rams = isset($_POST['rams']) ? explode(',', $_POST['rams']) : [];
            $almacenamientos = isset($_POST['almacenamientos']) ? explode(',', $_POST['almacenamientos']) : [];
            ...
            foreach ($cpus as $cpu_id) {
                if (!empty($cpu_id)) {
                    ...
                    if (sqlsrv_query($conn, $sql, [$id_laptop, $cpu_id]) === false) {
                        throw new Exception("Error inserting CPU");
                    }
                }
            }
            ...
            foreach ($rams as $ram_id) {
                if (!empty($ram_id)) {
                    ...
                    if (sqlsrv_query($conn, $sql, [$id_laptop, $ram_id]) === false) {
                        throw new Exception("Error inserting RAM");
                    }
                }
            }
            ...
            foreach ($almacenamientos as $almacenamiento_id) {
                if (!empty($almacenamiento_id)) {
                    ...
                    if (sqlsrv_query($conn, $sql, [$id_laptop, $almacenamiento_id]) === false) {
                        throw new Exception("Error inserting Almacenamiento");
                    }
                }
            }
            ...
            if (sqlsrv_query($conn, $sql_activo, [$id_laptop]) === false) {
                throw new Exception("Error inserting activo");
            }
        ...
        try {
            ...
            if ($row = sqlsrv_fetch_array($stmt_get_laptop, SQLSRV_FETCH_ASSOC)) {
                ...
                if (sqlsrv_query($conn, $sql_laptop, $params_laptop) === false) {
                    throw new Exception("Error updating laptop");
                }
                ...
                foreach ($cpus as $cpu_id) {
                    if (!empty($cpu_id)) {
                        sqlsrv_query($conn, "INSERT INTO laptop_procesador (id_laptop, id_cpu) VALUES (?, ?)", [$id_laptop, $cpu_id]);
                    }
                }

                foreach ($rams as $ram_id) {
                    if (!empty($ram_id)) {
                        sqlsrv_query($conn, "INSERT INTO laptop_ram (id_laptop, id_ram) VALUES (?, ?)", [$id_laptop, $ram_id]);
                    }
                }

                foreach ($almacenamientos as $almacenamiento_id) {
                    if (!empty($almacenamiento_id)) {
                        sqlsrv_query($conn, "INSERT INTO laptop_almacenamiento (id_laptop, id_almacenamiento) VALUES (?, ?)", [$id_laptop, $almacenamiento_id]);
                    }
                }
            }
