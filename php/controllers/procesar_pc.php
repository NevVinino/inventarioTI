...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($accion === "crear") {
        ...
        $slots_tarjeta_video = (isset($_POST["slots_tarjeta_video"]) && $_POST["slots_tarjeta_video"] !== '') ? (int)$_POST["slots_tarjeta_video"] : 1; // NUEVO
        ...
        if ($slots_tarjeta_video < 0 || $slots_tarjeta_video > 4) { // NUEVO
        ...
        try {
            // Insert PC first - REMOVE old component insertion
            ...
            if ($stmt_get_activo_id && $row_activo = sqlsrv_fetch_array($stmt_get_activo_id, SQLSRV_FETCH_ASSOC)) {
                ...
                if (!empty($slots_data)) {
                    ...
                    if (isset($slots_data['cpu']) && !empty($slots_data['cpu'])) {
                        error_log("Asignando CPU PC: " . $slots_data['cpu']);
        ...
        // Validar que el número de serie no exista en otro PC
        ...
        if ($row_serial['count'] > 0) {
            ...
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
                header('Content-Type: application/json');
                echo json_encode(['success' => false, 'error' => "El número de serie '$numberSerial' ya existe en otro PC. Por favor, ingrese un número de serie único."]);
                exit;
            }
            die("❌ Error: El número de serie '$numberSerial' ya existe en otro PC. Por favor, ingrese un número de serie único.");
        }
