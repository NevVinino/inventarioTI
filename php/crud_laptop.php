...
// Consultas para selects (ajustadas para la nueva estructura)
...
if ($empresas === false) {
    die("Error en consulta empresas: " . print_r(sqlsrv_errors(), true));
}
if ($marcas === false) {
    die("Error en consulta marcas: " . print_r(sqlsrv_errors(), true));
}
if ($estados === false) {
    die("Error en consulta estados: " . print_r(sqlsrv_errors(), true));
}

// Lista de laptops (ajustada para la nueva estructura)
$sql = "
SELECT 
    a.id_activo,
    a.tipo_activo,
...
<div id="modalActivo" class="modal" style="display:none;">
    <div class="modal-content">
        ...
        <!-- Modificar el formulario para ajustarse a la nueva estructura -->
        <form id="formActivo" method="POST" action="../controllers/procesar_laptop.php">
            ...
            <input type="hidden" name="tipo_activo" value="Laptop">
            ...
            <br>
            <?php
            // Función select modificada para ser más simple
            function select($name, $dataset, $id_field, $desc_field, $customLabel = null) {
                if ($dataset === false) return; // Evitar errores si la consulta falló
            ...
            // Solo mostrar los selects necesarios para la nueva estructura
